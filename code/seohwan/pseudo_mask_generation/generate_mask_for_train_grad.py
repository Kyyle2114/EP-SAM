...
def make_pseudo_mask(
    ...
    save_mask_dir = f'{output_path}/train_pseudo_masks_tiger_resnet_val'
    ...
    for X, y, _, file_name in tqdm(data_loader):
        ...
        for image, gt_mask, file in zip(X_torch, y_torch, file_name):
            ...
            #         'boxes': box_torch,
            #         'original_size': original_size,
            #         'cam_mask': box_torch, # whole bbox 그대로
            #         'file_name': file
        ...
        for output in batched_output:
            ...
            cam_mask.save(f'{save_dir}/train_cam_masks_tiger_resnet_val/{file}')
            
    df = pd.DataFrame(output_dict)
    df.to_csv(f'{save_dir}/train_pseudo_masks_tiger_resnet_val.csv')
