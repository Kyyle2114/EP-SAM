...
def make_point_mask(
    ...
    save_cam_dir = f'{save_dir}/train_point_prompt_cam_masks'
        ...
        for X, y, _, file_name in tqdm(data_loader):
            ...
            for image, gt_mask, file in zip(X_torch, y_torch, file_name):
                ...
                if logit.item() >= 0:
                    ...
                    # point from CAM 
                    cam_mask, softmax_cam = cls.make_cam(to_pil_image(torch.as_tensor(image, dtype=torch.uint8).squeeze()), device=device)
                    ...
                    point_coords = find_highest_proba_points(softmax_cam=softmax_cam, cam_mask=cam_mask, n_point=50)
                    # point_coords2 = make_proba_point_prompt(softmax_cam=softmax_cam, cam_mask=cam_mask, n_point=50)
            ...
            for output in batched_output:
                ...
                cam_mask.save(f'{save_cam_dir}/{file}')
