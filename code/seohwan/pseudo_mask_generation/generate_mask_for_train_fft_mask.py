        ...
        for X, y, _, file_name in tqdm(data_loader):
            ...
            for image, gt_mask, file in zip(X_torch, y_torch, file_name):
                ...
                if logit.item() >= 0:
                    ...
                    # cam_mask를 NumPy 배열로 변환
                    cam_mask_np = np.array(cam_mask)

                    # 256x256 크기로 리샘플링
                    mask_input = cv2.resize(cam_mask_np, (256, 256), interpolation=cv2.INTER_NEAREST)
            ...
            for output in batched_output:
                ...
                cam_mask.save(f'{save_dir}/train_mask_prompt_cam_masks_internel/{file}')
