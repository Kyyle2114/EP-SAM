...
from segment_anything.utils import trainer_point
...
from model import adl_fft, adl_fft_internel_point, adl_fft_internel
...
def main(rank, opts) -> str:
    ...
    es = trainer_point.EarlyStopping(patience=10, delta=0, mode='max', verbose=True)
    ...
    for epoch in range(EPOCHS):
        ...
        train_bce_loss, train_iou_loss, train_dice, train_iou = trainer_point.model_train(
            model=model,
        ...
        if opts.rank == 0:
            ...
            val_bce_loss, val_iou_loss, val_dice, val_iou = trainer_point.model_evaluate(
                model=model,
                cls=cls,
