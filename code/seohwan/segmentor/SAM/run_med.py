...
from segment_anything import build_sam_med
...
def get_args_parser():
    ...
    parser.add_argument('--port', type=int, default=1231, help='port number for distributed learning')
    ...
    parser.add_argument('--sam_decoder', type=str)
...
def main(rank, opts) -> str:
    ...
    sam = build_sam_med.sam_model_registry[model_type](checkpoint=sam_checkpoint)
    sam.cuda(local_gpu_id)
    
    sam = save_weight.load_partial_weight(
    model=sam,
    load_path=opts.sam_decoder,
    dist=False
)
    ...
    es = trainer_whole_bbox.EarlyStopping(patience=EPOCHS//2, delta=0, mode='min', verbose=True)
